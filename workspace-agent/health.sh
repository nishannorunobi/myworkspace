...
if [ -f "agent.conf" ]; then
    pass "agent.conf exists"
    source agent.conf 2>/dev/null || true
    [ -n "${ANTHROPIC_API_KEY:-}" ] \
        && pass "ANTHROPIC_API_KEY set (${ANTHROPIC_API_KEY:0:10}...)" \
        || fail "ANTHROPIC_API_KEY not set"
else
    fail "agent.conf not found — run ./build.sh"
fi

# ── memory/ dir ────────────────────────────────────────────────────────────────
if [ -d "memory" ]; then
    FILES=$(ls memory/ 2>/dev/null | wc -l)
    pass "memory/ exists ($FILES file(s))"
    [ "$FILES" -gt 0 ] && info "$(ls memory/)"
else
    warn "memory/ not found — will be created on first run"
fi
