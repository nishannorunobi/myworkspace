"""
Initspace proxy — forwards all /api/initspace/* requests to the workspace agent
running on port 8895. Returns 503 if the workspace agent is offline.

Mirrors routers/dockerspace.py but targets the workspace's init/ directory
(environment-initialization scripts such as git_clone_projects.sh).
"""
import json
import urllib.error
...
router = APIRouter(prefix="/initspace", tags=["initspace"])
...
def run_script(body: RunBody):
    ...
    return _stream("/api/initspace/run", payload)


@router.post("/kill")
def kill_script():
    d = _post("/api/initspace/kill")
    return d if d is not None else _offline()
