// ── TodayPanel rendering — pure render function ───────────────────────────────
...
function renderToday(d) {
  ...
  const todoRows = todos.length
    ? todos.map(t => `<div class="today-todo-item" id="todo-row-${t.id}">
        ...
        <button class="today-todo-done" onclick="window._dash.panels.today.completeTodo(${t.id})" title="Mark done">✓</button>
      </div>`).join('')
  ...
  const seg = (n, c) => n ? `<div class="today-prio-seg" style="width:${(n/prioTotal*100).toFixed(1)}%;background:${c}"></div>` : '';
  const lbl = (n, c, t) => n ? `<span style="color:${c}">● ${n} ${t}</span>` : '';
  const prioBar = todos.length ? `
    <div class="today-prio-bar">
      ${seg(prioCounts.urgent,'#f85149')}${seg(prioCounts.high,'#d29922')}${seg(prioCounts.normal,'#58a6ff')}${seg(prioCounts.low,'#3fb950')}
    </div>
    <div class="today-prio-legend">
      ${lbl(prioCounts.urgent,'#f85149','urgent')}${lbl(prioCounts.high,'#d29922','high')}${lbl(prioCounts.normal,'#58a6ff','normal')}${lbl(prioCounts.low,'#3fb950','low')}
    </div>` : '';
    ...
    ? [
        ...gitRepos.map(r => {
          ...
          return `<div class="today-proj-item">
            ...
            ${badges}
            <button class="today-proj-open git-btn" style="font-size:9px;padding:1px 5px" onclick="window._dash.panels.git.jumpTo('${esc(r.path)}');window._dash.nav.switchTab('git')">Open</button>
          </div>`;
        }),
