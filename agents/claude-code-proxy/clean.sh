#!/bin/bash
# clean.sh — Remove build artifacts so start.sh recreates a fresh environment.
# Removes : .venv, __pycache__, *.pyc, log files
# Preserves: (no user config files for this agent)
set -euo pipefail
...
echo -e "\n${BOLD}╔══════════════════════════════════════════╗${RESET}"
echo -e "${BOLD}║   Claude Code Proxy — Clean              ║${RESET}"
echo -e "${BOLD}╚══════════════════════════════════════════╝${RESET}\n"

# Stop agent if running
if pkill -f "$SCRIPT_DIR/.venv" 2>/dev/null; then
    ok "Stopped claude-code-proxy"
fi
